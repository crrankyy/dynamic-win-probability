"""
BiLSTM Training Script for IPL Win Probability
- Loads configs from configs/bilstm.yaml
- Sequences grouped by (match_id, innings)
- Expanding-window temporal CV (3 folds)
- Saves best model to models/bilstm.pth
"""
...
class MatchSequenceDataset(Dataset):
    def __init__(self, sequences, labels):
        self.sequences = sequences  # list of np arrays (seq_len, features)
...
def build_sequences(df, features, seq_length, scaler=None):
    ...
    for (mid, inn), group in df.groupby(['match_id', 'innings']):
        ...
        if len(seq) >= seq_length:
            seq = seq[-seq_length:]  # take the last seq_length balls
        else:
...
def main():
    ...
    config_path = os.path.join(PROJECT_ROOT, 'configs', 'bilstm.yaml')
