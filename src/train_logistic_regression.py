"""
Logistic Regression Training Script for IPL Win Probability
- Loads configs from configs/logistic_regression.yaml
- Expanding-window temporal CV (3 folds)
- Saves best model to models/logistic_regression.pkl
"""
...
TEMPORAL_CV_FOLDS = [
    (2019, 2020),  # train ≤ 2019, val = 2020
    (2020, 2021),  # train ≤ 2020, val = 2021
    (2021, 2022),  # train ≤ 2021, val = 2022
]
...
def load_data(subset='train'):
    ...
    df = pd.read_csv(path)
    df = df.fillna(0)  # Defensive: handle any NaN from feature extraction
    return df
...
def main():
    # Load config
    config_path = os.path.join(PROJECT_ROOT, 'configs', 'logistic_regression.yaml')
    ...
    final_model = make_pipeline(
        StandardScaler(),
        LogisticRegression(**best_config)
    )
